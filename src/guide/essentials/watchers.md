...
## Czyszczenie skutków ubocznych {#side-effect-cleanup}
...
Możemy użyć do tego API [`onWatcherCleanup()`](/api/reactivity-core#onwatchercleanup) <sup class="vt-badge" data-text="3.5+" />, aby zarejestrować funkcję czyszczącą, która wywoła się gdy obserwator będzie miał być zaraz wywołany z nową wartością:
...
Alternatywnie, funkcja `onCleanup` może również być przekazana do obserwatora jako trzeci argument<span class="composition-api">, a do funkcji `watchEffect` jako pierwszy argument</span>:
